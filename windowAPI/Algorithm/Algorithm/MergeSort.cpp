...
int main()
{
	...
	// std::sort(v.begin(), v.end()); // N X logN -> 빠른 정렬

	// std::partial_sort(v.begin(), v.begin() + 3, v.end()); // N X logN -> 앞에 3개만 최대값으로 정렬

	// std::stable_sort(v.begin(), v.end()); N X logN -> 힙 정렬
	// 안전한 정렬 (같은 값일 겨우 앞에 있던 수가 무조건 앞에 있음)
